...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.VigenereButton.grid(row=5, column=0, sticky="ns", pady=5)
        self.elbatal = customtkinter.CTkButton(self.navigation_frame, corner_radius=0, height=40,
                                                      border_spacing=10,
                                                      text="S.Elbatal",
                                                      fg_color="transparent", text_color=("gray10", "gray90"),
                                                      hover_color=("gray70", "gray30"),
                                                      image=self.logo_image, anchor="w",
                                                      command=self.elbatal_button_even)
        self.elbatal.grid(row=5, column=1, sticky="ns", pady=5)
        ...
        self.elbatal = customtkinter.CTkFrame(self, corner_radius=0, fg_color="transparent")
        ...
        self.create_gui_elements(self.elbatal, "Elbatal")
        self.Rot13.keyTextBox.configure(state="readonly")
        self.elbatal.keyTextBox.configure(state="readonly")
#==============> Edit process button name
        ...
        self.bind_update_process_button_text(self.elbatal)
#================================================================>>
        ...
        self.elbatal.processButton.configure(command= self.elbatalFunction)
        ...
        self.elbatal.selectFileButton.configure(command=lambda:self.add_content(self.elbatal))
    ...
    # ============================================================================
        ...
        self.elbatal.downloadButton.configure(command=lambda: self.download(self.elbatal))
    #===============================================================
        # ==> Functions
    def elbatalEncryption(self, text):
        tree = {}
        cypher = "ahals"
        global indx
        indx= 0  # Define indx here
        def prepare():
            sz = len(cypher)
        ...
        def decrypt(indx=1):
            ...
            if indx > len(cypher):
                return
            tree[indx].append(indx * 2)
            tree[indx].append(indx * 2 + 1)
            decrypt(indx * 2)
            decrypt(indx * 2 + 1)
        ...
        for i in tree:
            for j in tree[i]:
                plain_text += j
        return plain_text
    def elbatalDecryption(self,text):
        plain_text = ""
        ...
        def prepare():
            for i in range(0, len(plain_text) * 2 + 1):
                tree[i] = []
        ...
        return txt
    #
    # if plain_text=="fefeslfmlksemfkselmflkesmflksemflksemlfkmseklfmselkmflksemlkfmlksemflkmseklfmlksemflkesmflkmselkfmleskmflksemflkemkflmselkfmselkfmlksefsefse":
    #     print("Valid ")
    ...
    def VigenereFunction(self):
        ...
        if len(key) > len(PlainText):
            messagebox.showerror("Error","Invalid key. The key length should be less than or equal to the plaintext or ciphertext length.")
            return
    ...
    def encryptionVigenereCipher(self, plainText, key):
        if len(key) == 0:
            messagebox.showerror("Error", "Invalid key. The key should not be empty.")
            return
    ...
    def elbatalFunction(self):
        # Get the ciphertext and key from the input fields
        self.elbatal.keyTextBox.configure(state="readonly")
        self.elbatal.decryptTextBox.delete(0, "end")
        PlainText = self.elbatal.encryptTextBox.get()
        rVar = self.elbatal.radioVar.get()

        if rVar == 1:
            decryptedText = self.elbatalEncryption(PlainText)
            self.elbatal.decryptTextBox.insert("end", decryptedText)
        else:

            encryptedText = self.elbatalDecryption(PlainText)
            self.elbatal.decryptTextBox.insert("end", encryptedText)
    ...
    def TranspositionFunction(self):
        ...
        if rVar == 0:
            # Validate plaintext
            if not all(char.isalpha() for char in PTxt):  # Adjusted condition here
                messagebox.showerror("Error",
                                     "Invalid plaintext. Please ensure the plaintext is alphabetic (spaces not allowed).")
                return
...
#==============================================================>
    ...
    def select_frame_by_name(self, name):
        ...
        if name == "elbatal":
            self.elbatal.grid(row=0, column=1, sticky="nsew")
        else:
            self.elbatal.grid_forget()
    ...
    def elbatal_button_even(self):
        self.select_frame_by_name("elbatal")
